...
export async function generateMetadata({
  ...
  if (!product)
    return { title: "Piece Not Found | Corals by Tabassum" };
  return {
    title: `${product.name} | Corals by Tabassum`,
...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  const specRows = product.specifications
    ...
    : [
        ...
        { label: "Metal", value: product.movement ?? "Gold Plated" },
        { label: "Material", value: product.strapMaterial ?? "—" },
        { label: "Gemstone", value: product.caseMaterial ?? "—" },
        { label: "Size", value: product.caseDiameter ?? "—" },
        { label: "Weight", value: product.waterResistance ?? "—" },
        { label: "Craft", value: product.displayType ?? "—" },
        { label: "Warranty", value: product.warranty ?? "6 Months" },
        { label: "Style", value: product.occasion ?? "Daily Wear" },
      ];
  ...
  const sidebarSpecs = specRows
    .filter((s) =>
      ["Metal", "Gemstone", "Size", "Material", "Weight", "Warranty"].includes(
        s.label
      )
    )
    .slice(0, 6);
  ...
  return (
    <div className="container-tc py-8 lg:py-12">
      ...
      <nav className="mb-6 flex flex-wrap items-center gap-1.5 text-xs text-text-gray" aria-label="Breadcrumb">
        ...
        <ChevronRight className="h-3 w-3" />
        <Link href={`/shop?category=${product.category.slug}`} className="hover:text-champagne">
          ...
          <div className="rounded-2xl border border-soft-gray bg-white p-7 shadow-sm shadow-obsidian/5">
            ...
            {outOfStock && (
              <div className="mt-4 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">
                This piece is currently out of stock. Check back soon or add it
