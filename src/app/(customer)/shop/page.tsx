...
export default async function WatchesPage({ searchParams }: WatchesPageProps) {
  ...
  const title = q
    ? `Results for "${q}"`
    : category
      ? (categories.find((c) => c.slug === category)?.name ?? "Jewellery")
      : "All Jewellery";
  ...
  const active = (() => {
    ...
    if (q) {
      ...
      chips.push({ label: `"${q}"`, href: `/shop?${p}` });
    }
    if (category)
      chips.push({
    ...
    <div className="container-tc py-10 lg:py-14">
      <div className="pt-10 text-center lg:pt-16">
        <p className="text-xs font-light uppercase tracking-[0.45em] text-champagne">
          Corals by Tabassum &mdash; The Collection
        </p>
        ...
        <p className="mx-auto mt-6 max-w-xl text-sm font-light leading-relaxed tracking-wide text-text-gray">
          The Corals by Tabassum collection offers a wide range of jewellery &mdash;
          from everyday classics to statement pieces &mdash; for every hand, every aesthetic.
        </p>
