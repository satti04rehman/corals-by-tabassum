...
**Elegance meets everyday** — a premium jewellery e-commerce storefront built with Next.js 16 (App Router), Supabase (Postgres / Auth / Storage), Prisma 7, Tailwind CSS 4, and Recharts. Cream, blush and rose-gold theme with Inter + Playfair Display.

## Features

- **Home** — editorial cream-and-ink hero, marquee, featured pieces, brand bands and testimonial sections
- **Catalog** — `/shop` with URL-driven filters (category, brand, gender, price, rating, metal, style, colour), sorting, and pagination
- **Product detail** — `/shop/[slug]` with gallery, add-to-cart / buy-now, tabs (description / specs / reviews), and related products
...
- **Piece Finder** — `/find-piece` rule-based quiz that recommends jewellery from the catalog
...
- Catalog, search, and home content come from `src/lib/demo-data.ts` (local jewellery photography)
...
The seed creates an admin profile (email from `SEED_ADMIN_EMAIL`, default `hello@coralsbytabassum.com`) on the `users`/`profiles` tables.
